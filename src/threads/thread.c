...
void recalculate_BSD_variables(void) {
    ASSERT(thread_mlfqs);
...
bool priority_less_func(const struct list_elem *a,
    ...
    a_ready = list_entry(a,
    struct thread, elem);
    b_ready = list_entry(b,
    struct thread, elem);
...
void thread_set_priority_extra(struct thread *curr, int new_priority, bool force) {
    if (!curr->donated)
